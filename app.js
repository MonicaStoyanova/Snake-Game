document.addEventListener("DOMContentLoaded", () => {
  ...
  let appleIndex = 0;
  let currentSnake = [2, 1, 0]; //so the div in our grid being 2(or the HEAD), and 0 being the wnd(TAIL, with all 1`s being the body from now on)
  let direction = 1;
  let score = 0;
  let speed = 0.9;
  let intervalTime = 0;
  ...
  function randomApple() {
    ...
    } while (squares[appleIndex].classList.contains("snake")); //making sure apples do not appear on the snake
    squares[appleIndex].classList.add("apple");
  }

  //assign functions to keycodees
  function control(e) {
